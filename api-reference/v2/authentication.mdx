...
<Steps>
  <Step title="Войдите в кабинет">
    [Панель управления → Профиль](https://loveandpay.io/dashboard/profile)
  </Step>
  <Step title="API Key (публичный)">
...
<Note>
**Формат секрета.** Секретный ключ v2 — это строка из **64 hex-символов** (без префикса),
например `65a272ce7f5e...cced3bcaf14`. В примерах кода `sk_live_...` — лишь плейсхолдер;
подставляйте ваш реальный ключ из кабинета.
</Note>

<Note>
**Ключ всегда в паре с секретом.** Если по вашему `x-api-key` секрет в системе
не настроен — запрос вернёт `SECRET_NOT_CONFIGURED`; сгенерируйте секрет в кабинете
(создаётся в паре с ключом). Секрет в запросе **не передаётся** — сервер сверяет
подпись с хранимым секретом.
</Note>

<Note>
**Под-партнёры.** v2 поддерживает как партнёрский ключ, так и **личный ключ
под-партнёра** (выдаётся в его кабинете) — он подписывает запросы **своим** секретом,
операции закрепляются за ним. Новый под-партнёр должен быть **активирован
администратором** (назначены терминал и комиссия), иначе создание счетов вернёт
`PARTNER_PENDING_MODERATION`.
</Note>
