...
<ParamField body="paymentMethod" type="string" default="sbp">
  Метод оплаты: `"sbp"` (СБП, по умолчанию) или `"card"` (банковская карта через Overpay). Подробнее — [Создать счёт (оплата по карте)](/api-reference/v2/invoices/create-card).
</ParamField>
