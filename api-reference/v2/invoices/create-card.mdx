...
description: 'Создание счёта с оплатой банковской картой'
...
| `originalPaymentUrl` | Прямая ссылка на платёжную форму |
...
console.log('Прямая ссылка:', data.invoice.originalPaymentUrl);
```
...
print('Прямая ссылка:', data['invoice']['originalPaymentUrl'])
```
</RequestExample>
...
| **Прямая ссылка на форму** | — | `originalPaymentUrl` |
