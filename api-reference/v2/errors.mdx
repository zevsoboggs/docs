...
| Код | Описание | Решение |
|----|----------|---------|
| `PARTNER_INACTIVE` | Аккаунт неактивен / на модерации | Дождитесь активации админом или обратитесь в поддержку |
| `PARTNER_PENDING_MODERATION` | Под-партнёр ещё не активирован (модерация) | Админ должен назначить терминал и активировать аккаунт |
| `PARTNER_REJECTED` | Аккаунт отклонён администратором | Обратитесь в поддержку |
...
| `NO_TERMINAL` | Партнёру не назначен терминал/провайдер | Админ должен назначить терминал |
| `QR_UNAVAILABLE` | Не удалось получить QR у провайдера | Повторите запрос; если повторяется — поддержка |
